...
This skill is limited to Matt's current Teal operations. It may produce competitor evidence, workflow observations, and provider-adapter requirements, but it may not define the customer career platform's domain model, routes, screen system, or MVP dependencies. Product architecture must remain native and functional without Teal.
...
- Compensation answers must preserve leverage. Never guess a low hourly number just to satisfy an application form. If the field accepts text, prefer `Negotiable` unless Matt gave a different instruction. For hospitality roles where Matt has approved numeric fallback handling, if the field requires a number, use the active posting's pay evidence first. If no role-specific pay is visible, use current market evidence for that exact lane, such as barback, server assistant, server, host, or food runner, and note the source after submission. Never use `$18/hr` as a blind hospitality fallback unless the active posting itself shows that rate or Matt explicitly approves it for that exact role.
- For Nashville hospitality resumes, use only the approved local PDFs unless a live Teal export is created for that exact role. Use `applications/hospitality-lanes/Matt Dimock - Hospitality Resume.pdf` for server, server-track, and general hospitality roles. Use `applications/hospitality-lanes/Matt Dimock - Barback Resume.pdf` for barback and bartender-support roles. Use `applications/hospitality-lanes/Matt Dimock - Server Resume.pdf` and `applications/hospitality-lanes/Matt Dimock - Barback Upload Resume.pdf` only as reusable non-branded upload copies of those approved resumes. Do not use older Broadway, Nashville Broadway, baseline, food runner, server assistant, or experimental hospitality resume drafts unless Matt explicitly approves that exact fallback. Preserve the approved summary structure, full chronology, The Farm House Restaurant entry, and selected hospitality-transfer bullets across all roles. Job-specific optimization may adjust the target title and truthful hard/soft skill phrasing for the posting; only adjust the summary when the target job clearly requires it or Matt approves it. Do not activate a skills dump. Passing pre-export verification means every active bullet has a clear hospitality-transfer reason and the preview remains readable.
...
18. Reduce hospitality resume length in this order: uncheck weak or repetitive bullets, uncheck position descriptions, clean duplicate skills, then adjust Designer layout. Exclude roles only when Matt explicitly approves a shortened timeline.
